...
def make_bar_graph(df,xval,yval,color1,cs1,title_, s_title):
    ...
    fig.update_layout(font_family="Balto",font=dict(size=45), margin=dict(l=30, r=30, t=30, b=0))
    fig.update_layout(title_font_weight=1000, title_subtitle_text=s_title)
    fig.update_layout(title_subtitle_font_size= 13)
    fig.update_layout(title_font_size= 17)
    fig.update_xaxes(rangeselector_font_size=50,autotickangles=[30])
    return fig
...
with col1:       
    ...
    fig = make_bar_graph(all_scopes, xval= "Fiscal_Year" ,yval= "MTCDE", color1="Emissions_Source", cs1=px.colors.qualitative.Safe, title_= "Middlebury College Greenhouse Gas Inventory: 2006-2024",s_title= "More info here")         
    st.plotly_chart(fig)#, use_container_width=True)


   
    st.write ("Double click on any variable in the legend to isolate it on the graph")
    fig1=make_bar_graph(scope1, xval= "Fiscal_Year" ,yval= "MTCDE", color1="Emissions_Source", cs1=px.colors.qualitative.Bold, title_= "Middlebury College Greenhouse Gas Inventory: Scope 1", s_title= "More info here")                
    ...
    st.write("Double click on any box in the legend visual to isolate it on the graph")
    fig1=make_bar_graph(scope2, xval= "Fiscal_Year" ,yval= "MTCDE", color1="Emissions_Source", cs1=px.colors.qualitative.Set2_r, title_= "Middlebury College Greenhouse Gas Inventory: Scope 2", s_title= "More info here")                
    st.plotly_chart(fig1,use_container_width=True)
        
  
    st.write("Double click on any box in the legend visual to isolate it on the graph")

    fig1=make_bar_graph(scope3[scope3["Emissions_Source"]!="Total"], xval= "Fiscal_Year" ,yval= "MTCDE", color1="Emissions_Source", cs1=px.colors.qualitative.D3_r, title_= "Middlebury College Greenhouse Gas Inventory: Scope 3",s_title= "More info here")                
    st.plotly_chart(fig1,use_container_width=True)
...
with col2:
    ...
    st.markdown("**Scope 1A**: ")
    st.markdown("**Scope 1B**: ")
    
    #need to align content 
    # https://discuss.streamlit.io/t/alignment-of-content/29894/3
